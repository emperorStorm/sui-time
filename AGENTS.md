...
- 移动端：`uni-client/` 为 UniApp（Vue 3、纯 JS、rpx）安卓客户端，数据用本地存储独立实现，与桌面 SQLite 不互通；APK 由 CI 离线打包自动构建（`android-apk.yml`，tag 触发），模板在 `uni-client/android-templates/`。
...
普通修改不自动创建变更 Spec 或其他过程文档；仅在用户明确要求时新增项目文档。

...
- macOS 使用 `UNUserNotificationCenter` 查询真实的未询问、拒绝、已允许和系统错误状态；首次开启提醒先显示应用内说明，再由用户主动触发系统授权框，授权请求可等待用户完成系统选择。
- 已拒绝时保持提醒关闭，并提供 macOS 系统通知设置入口；应用不能写入系统通知列表、绕过授权或重置用户的拒绝决定。授权成功后自动发送激活通知，并保留测试通知入口确认系统横幅可见。

...
事项可按日、间隔日、周、工作日、月、年、记忆曲线或自定义槽位重复，并允许对某一次实例完成、移动、编辑或删除，而不改写其他实例。
...
- 单次完成、移动和删除只改变对应 override；基础重复事项保持 `todo`。
...
- 覆盖各规则日期匹配、结束条件、移动实例、删除实例和重复事项全局完成态归零。
- Rust 校验 JSON 合法性并测试 v5 迁移；前端工具函数是实例展开真值。

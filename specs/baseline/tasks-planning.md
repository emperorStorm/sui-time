...
- 子事项的新增、标题、完成状态和删除随父事项弹窗底部“保存”或子事项状态按钮的即时保存通过同一事务写入；取消弹窗不修改已保存的子事项。
- 保存普通父事项的子事项后，存在至少一个子事项且全部完成时父事项自动完成；任一子事项待完成且父事项原为完成时恢复待完成。失败父事项、重复事项、嵌套子事项和零子事项不参与此汇总。
- 任务弹窗的总事项勾选按钮与子事项状态按钮会先按底部“保存”的校验规则写入当前有效草稿，并保持弹窗打开；总事项完成和恢复的既有状态语义不变，重复事项单次实例继续使用 occurrence override。
